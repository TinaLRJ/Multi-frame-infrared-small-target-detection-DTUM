...
def writeNUDTMIRSDT_ROC(FalseNumBatch, TrueNumBatch, TgtNumBatch, pixelsNumBatch, Th_Seg, test_txt, save_path, args, epoch):
    ...
    pixelsNumBatch = [x.item() if torch.is_tensor(x) else x for x in pixelsNumBatch]
    print("pixelsNumBatch type:", type(pixelsNumBatch))
    print("pixelsNumBatch length:", len(pixelsNumBatch))
    print("Example item:", pixelsNumBatch[0])
    print("Item types:", [type(x) for x in pixelsNumBatch[:10]])
...
def writeCSIG_new_ROC(FalseNumBatch, TrueNumBatch, TgtNumBatch, pixelsNumBatch, Th_Seg, save_path, args, epoch):
...
def writeCSIG_ROC(FalseNumBatch, TrueNumBatch, TgtNumBatch, pixelsNumBatch, Th_Seg, test_txt, save_path, args, epoch):
    # low_snr3 = [47, 56, 59, 76, 92, 101, 105, 119]
    # high_snr3 = [85, 86, 87, 88, 89, 90, 91, 93, 94, 95, 96, 97]
    # seq_names = list(dict.fromkeys([int(x.split('/')[0].split('Sequence')[1]) for x in test_txt]))
    # index = [seq_names.index(i) for i in low_snr3+high_snr3]
    ...
    seq_names = list(dict.fromkeys([x.split('/')[3] for x in test_txt]))
    index = range(len(seq_names))

    FalseNumAll = np.array(FalseNumBatch).reshape((len(seq_names), -1, len(Th_Seg))).sum(axis=1)
    TrueNumAll = np.array(TrueNumBatch).reshape((len(seq_names), -1, len(Th_Seg))).sum(axis=1)
    TgtNumAll = np.array(TgtNumBatch).reshape((len(seq_names), -1, len(Th_Seg))).sum(axis=1)
    pixelsNumBatch = [x.item() if torch.is_tensor(x) else x for x in pixelsNumBatch]
    print("pixelsNumBatch type:", type(pixelsNumBatch))
    print("pixelsNumBatch length:", len(pixelsNumBatch))
    print("Example item:", pixelsNumBatch[0])
    print("Item types:", [type(x) for x in pixelsNumBatch[:10]])
    pixelsNumber = np.array(pixelsNumBatch).reshape(len(seq_names), -1).sum(axis=1)
    ...
    for i in range(len(seq_names)):
        seq = seq_names[i]
        writelines.write('val' + seq + 'results:\n')
        for seg_i in range(len(Th_Seg)):
            writelines.write('Th_Seg = %e:\tPD:[%d/%d, %.5f]\tFA:[%d, %e]\n' % (
                Th_Seg[seg_i], TrueNumAll[index[i], seg_i], TgtNumAll[index[i], seg_i],
                TrueNumAll[index[i], seg_i] / TgtNumAll[index[i], seg_i], FalseNumAll[index[i], seg_i],
