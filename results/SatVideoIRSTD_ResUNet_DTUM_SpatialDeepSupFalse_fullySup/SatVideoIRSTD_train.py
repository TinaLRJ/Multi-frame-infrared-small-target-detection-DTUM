...
from SatVideoIRSTD_DataLoader import CSIG_TrainSetLoader, CSIG_TestSetLoader
...
from SatVideoIRSTD_write_results import writeNUDTMIRSDT_ROC, writeCSIG_new_ROC, writeCSIG_ROC
...
def parse_args():
    ...
    parser.add_argument('--dataset',   type=str, default='CSIG_new', help='Dataset name [dafult: CSIG]')        #修改001
    ...
    parser.add_argument('--saveDir',   type=str, default='./results_CSIG_new/',
                            help='Save path [defaule: ./results_CSIG/]')    #修改003
    parser.add_argument('--train',    type=int, default=0)
    parser.add_argument('--test',     type=int, default=1)
    parser.add_argument('--pth_path', type=str, default='results/SatVideoIRSTD_ResUNet_DTUM_SpatialDeepSupFalse_fullySup/ResUNet_DTUM.pth', help='Trained model path')
...
class Trainer(object):
    def __init__(self, args):
        ...
        elif args.dataset == 'CSIG_new':    #修改002
            self.train_dataset = CSIG_TrainSetLoader(train_path, fullSupervision=args.fullySupervised)
            self.val_dataset = CSIG_TestSetLoader(self.test_path)
    ...
    def training(self, epoch):
        ...
        loss_last = 0.0
        self.net.train()
        num_iters = len(self.train_loader) // 10
    ...
    def validation(self, epoch):
        args = self.args
        txt = np.loadtxt(self.test_path + 'val_list.txt', dtype=bytes).astype(str)
        self.net.eval()
        ...
        for i, data in enumerate(tqdm(self.val_loader), 0):
            ...
            with torch.no_grad():
                ...
                if self.save_flag:
                    ...
                    txt[i] = txt[i].replace('\\', '/')
                    folder_name = "%s%s/" % (self.test_save, txt[i].split('/')[3])
        ...
        if self.writeflag:
            ...
            elif 'CSIG_new' in args.dataset:
                writeCSIG_new_ROC(FalseNumBatch, TrueNumBatch, TgtNumBatch, pixelsNumBatch, Th_Seg, self.SavePath, args, epoch)
